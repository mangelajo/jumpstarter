...
class Cuttlefish(Driver):
    ...
    def _do_cvd_operation(self, method: str, suffix: str = "", data: dict | None = None):
        # Resolve the identity under the same lock that publishes create results.
        with self._operation_lock:
            return self._do_operation(method, self._cvd_path + suffix, data)
    ...
    def start_cvd(self) -> str:
        return self._fmt(self._do_cvd_operation("POST", "/:start", {}))

    @export
    def stop_cvd(self) -> str:
        return self._fmt(self._do_cvd_operation("POST", "/:stop"))

    @export
    def delete_cvd(self) -> str:
        return self._fmt(self._do_cvd_operation("DELETE"))
